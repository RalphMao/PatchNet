...
class CenterLoss(nn.Module):
    ...
    def _get_center_thresh(self, size, center):
        weight = torch.zeros(2, size, size).float()
        weight[0] = torch.arange(size, dtype=torch.float) - center[1]
        weight[1] = torch.arange(size, dtype=torch.float)[:, None] - center[0]
        weight = weight.abs().sum(dim=0)
        weight = 2 - torch.relu(2 - weight)
        return weight

    def forward(self, input, center=None):
        size = input.shape[-1]
        k = (size - 1) // 2
        if center is None:
            center = (k, k)
        center_val = input[..., center[1]:center[1]+1, center[0]:center[0]+1]
        thresh = self._get_center_thresh(size, center).to(input.device)
        loss = torch.relu(input + thresh - center_val).sum()
        return loss
